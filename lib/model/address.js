...
Address._deriveAddress = function(scriptType, publicKeyRing, path, m, coin, network) {
  ...
  switch (scriptType) {
    case Constants.SCRIPT_TYPES.P2SH:
      fcoreAddress = Fcash[coin].Address.createMultisig(publicKeys, m, network);
      break;
    case Constants.SCRIPT_TYPES.P2PKH:
      $.checkState(_.isArray(publicKeys) && publicKeys.length == 1);
      fcoreAddress = Fcash[coin].Address.fromPublicKey(publicKeys[0], network);
      break;
  }

  return {
    address: fcoreAddress.toString(),
    path: path,
