...
Address._deriveAddress = function(scriptType, publicKeyRing, path, m, coin, network) {
  ...
  var fcashBaseAddress;
  switch (scriptType) {
    case Constants.SCRIPT_TYPES.P2SH:
      fcashBaseAddress = Fcash[coin].Address.createMultisig(publicKeys, m, network);
      break;
    case Constants.SCRIPT_TYPES.P2PKH:
      $.checkState(_.isArray(publicKeys) && publicKeys.length == 1);
      fcashBaseAddress = Fcash[coin].Address.fromPublicKey(publicKeys[0], network);
      break;
  }

  return {
    address: fcashBaseAddress.toString(),
    path: path,
