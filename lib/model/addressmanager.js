...
AddressManager.create = function(opts) {
  ...
  x.fcash-payIndex = _.isNumber(opts.fcash-payIndex) ? opts.fcash-payIndex : Constants.BIP45_SHARED_INDEX;
...
AddressManager.fromObj = function(obj) {
  ...
  x.fcash-payIndex = obj.fcash-payIndex;

  return x;
};

AddressManager.supportsFcashAppBranches = function(derivationStrategy) {
...
AddressManager.prototype.getBaseAddressPath = function(isChange) {
  return 'm/' +
    (this.derivationStrategy == Constants.DERIVATION_STRATEGIES.BIP45 ? this.fcash-payIndex + '/' : '') +
    (isChange ? 1 : 0) + '/' +
    0;
};
