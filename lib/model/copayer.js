...
FcashApp._xPubToFcashAppId = function(coin, xpub) {
  var str = coin == Defaults.COIN ? xpub : coin + xpub;
  var hash = sjcl.hash.sha256.hash(str);
  return sjcl.codec.hex.fromBits(hash);
};

FcashApp.create = function(opts) {
  opts = opts || {};
  $.checkArgument(opts.xPubKey, 'Missing fcash-pay extended public key')
    .checkArgument(opts.requestPubKey, 'Missing fcash-pay request public key')
    .checkArgument(opts.signature, 'Missing fcash-pay request public key signature');

  $.checkArgument(Utils.checkValueInCollection(opts.coin, Constants.COINS));

  opts.fcash-payIndex = opts.fcash-payIndex || 0;
  ...
  x.id = FcashApp._xPubToFcashAppId(opts.coin, x.xPubKey);
  x.name = opts.name;
  ...
  if (AddressManager.supportsFcashAppBranches(derivationStrategy)) {
    x.addressManager = AddressManager.create({
      derivationStrategy: derivationStrategy,
      fcash-payIndex: opts.fcash-payIndex,
    });
  }
...
FcashApp.prototype.createAddress = function(wallet, isChange) {
