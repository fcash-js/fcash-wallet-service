...
FcashApp.create = function(opts) {
  opts = opts || {};
  $.checkArgument(opts.xPubKey, 'Missing fcashpay extended public key')
    .checkArgument(opts.requestPubKey, 'Missing fcashpay request public key')
    .checkArgument(opts.signature, 'Missing fcashpay request public key signature');

  $.checkArgument(Utils.checkValueInCollection(opts.coin, Constants.COINS));

  opts.fcashpayIndex = opts.fcashpayIndex || 0;
  ...
  if (AddressManager.supportsFcashAppBranches(derivationStrategy)) {
    x.addressManager = AddressManager.create({
      ...
      fcashpayIndex: opts.fcashpayIndex,
    });
  }
