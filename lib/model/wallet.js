...
Wallet.fromObj = function(obj) {
  ...
  x.fcashpay = _.map(obj.fcashpay, function(fcashpay) {
    return FcashApp.fromObj(fcashpay);
  });
  x.pubKey = obj.pubKey;
...
/**
 * Get the maximum allowed number of required fcashpay.
 * This is a limit imposed by the maximum allowed size of the scriptSig.
 * @param {number} totalFcashApp - the total number of fcashpay
 * @return {number}
 */
...
Wallet.prototype._updatePublicKeyRing = function() {
  this.publicKeyRing = _.map(this.fcashpay, function(fcashpay) {
    return _.pick(fcashpay, ['xPubKey', 'requestPubKey']);
  });
};

Wallet.prototype.addFcashApp = function(fcashpay) {
  $.checkState(fcashpay.coin == this.coin);

  this.fcashpay.push(fcashpay);
  if (this.fcashpay.length < this.n) return;
...
Wallet.prototype.addFcashAppRequestKey = function(FcashPayId, requestPubKey, signature, restrictions, name) {
  $.checkState(this.fcashpay.length == this.n);
