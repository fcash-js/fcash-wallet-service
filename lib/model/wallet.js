...
var FcashApp = require('./fcash-pay');
...
Wallet.fromObj = function(obj) {
  ...
  x.fcash-pay = _.map(obj.fcash-pay, function(fcash-pay) {
    return FcashApp.fromObj(fcash-pay);
  });
  x.pubKey = obj.pubKey;
...
/**
 * Get the maximum allowed number of required fcash-pay.
 * This is a limit imposed by the maximum allowed size of the scriptSig.
 * @param {number} totalFcashApp - the total number of fcash-pay
 * @return {number}
 */
Wallet.getMaxRequiredFcashApp = function(totalFcashApp) {
  return Wallet.COPAYER_PAIR_LIMITS[totalFcashApp];
};

Wallet.verifyFcashAppLimits = function(m, n) {
...
Wallet.prototype._updatePublicKeyRing = function() {
  this.publicKeyRing = _.map(this.fcash-pay, function(fcash-pay) {
    return _.pick(fcash-pay, ['xPubKey', 'requestPubKey']);
  });
};

Wallet.prototype.addFcashApp = function(fcash-pay) {
  $.checkState(fcash-pay.coin == this.coin);

  this.fcash-pay.push(fcash-pay);
  if (this.fcash-pay.length < this.n) return;

  this.status = 'complete';
  this._updatePublicKeyRing();
};

Wallet.prototype.addFcashAppRequestKey = function(FcashPayId, requestPubKey, signature, restrictions, name) {
  $.checkState(this.fcash-pay.length == this.n);

  var c = this.getFcashApp(FcashPayId);
...
Wallet.prototype.getFcashApp = function(FcashPayId) {
  return _.find(this.fcash-pay, {
    id: FcashPayId
  });
};
