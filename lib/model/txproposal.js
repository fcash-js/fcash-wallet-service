 ...
 * getActors
 *
 * @return {String[]} FcashPayIds that performed actions in this proposal (accept / reject)
 */
TxProposal.prototype.getActors = function() {
  return _.pluck(this.actions, 'FcashPayId');
};
 ...
 * getApprovers
 *
 * @return {String[]} FcashPayIds that approved the tx proposal (accept)
 */
 ...
 * getActionBy
 *
 * @param {String} FcashPayId
 * @return {Object} type / createdOn
 */
TxProposal.prototype.getActionBy = function(FcashPayId) {
  return _.find(this.actions, {
    FcashPayId: FcashPayId
  });
};

TxProposal.prototype.addAction = function(FcashPayId, type, comment, signatures, xpub) {
...
TxProposal.prototype._addSignaturesToFcashTx = function(tx, signatures, xpub) {
  ...
  var fcashBase = Fcash[self.coin];

  if (signatures.length != this.inputs.length)
    throw new Error('Number of signatures does not match number of inputs');

  var i = 0,
    x = new fcashBase.HDPublicKey(xpub);

  _.each(signatures, function(signatureHex) {
    var input = self.inputs[i];
    try {
      var signature = fcashBase.crypto.Signature.fromString(signatureHex);
      var pub = x.deriveChild(self.inputPaths[i]).publicKey;
      var s = {
        inputIndex: i,
        signature: signature,
        sigtype: fcashBase.crypto.Signature.SIGHASH_ALL | fcashBase.crypto.Signature.SIGHASH_FORKID,
        publicKey: pub,
      };
...
TxProposal.prototype.sign = function(FcashPayId, signatures, xpub) {
  try {
    // Tests signatures are OK
    var tx = this.getFcashTx();
    this._addSignaturesToFcashTx(tx, signatures, xpub);

    this.addAction(FcashPayId, 'accept', null, signatures, xpub);
...
TxProposal.prototype.reject = function(FcashPayId, reason) {
  this.addAction(FcashPayId, 'reject', reason);
};
