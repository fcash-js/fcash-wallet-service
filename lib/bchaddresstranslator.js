...
// Supports 3 formats:  legacy (1xxx, mxxxx); FcashApp: (Cxxx, Hxxx), Cashaddr(qxxx);
BCHAddressTranslator.translate = function(addresses, to, from) {
  ...
  var ret =  _.map(addresses, function(x) {
    ...
    var fcash_base = Fcash_[from == 'legacy' ? 'btc' : 'bch'];
    var orig = new fcash_base.Address(x).toObject();
