...
var collections = {
  ...
  COPAYERS_LOOKUP: 'fcash-pay_lookup',
...
Storage.prototype.storeWalletAndUpdateFcashAppLookup = function(wallet, cb) {
  var self = this;

  var fcash-payLookups = _.map(wallet.fcash-pay, function(fcash-pay) {
    $.checkState(fcash-pay.requestPubKeys);
    return {
      FcashPayId: fcash-pay.id,
      walletId: wallet.id,
      requestPubKeys: fcash-pay.requestPubKeys,
    };
  });
  ...
  this.db.collection(collections.COPAYERS_LOOKUP).remove({
    ...
    if (err) return cb(err);
    self.db.collection(collections.COPAYERS_LOOKUP).insert(fcash-payLookups, {
      w: 1
    }, function(err) {
      if (err) return cb(err);
...
Storage.prototype.fetchFcashAppLookup = function(FcashPayId, cb) {
...
Storage.prototype._completeTxData = function(walletId, txs, cb) {
  ...
  self.fetchWallet(walletId, function(err, wallet) {
    ...
    _.each([].concat(txs), function(tx) {
      ...
      tx.creatorName = wallet.getFcashApp(tx.creatorId).name;
      _.each(tx.actions, function(action) {
        action.fcash-payName = wallet.getFcashApp(action.FcashPayId).name;
      });
...
Storage.prototype.fetchPreferences = function(walletId, FcashPayId, cb) {
...
Storage.prototype.storePreferences = function(preferences, cb) {
  this.db.collection(collections.PREFERENCES).update({
    ...
    FcashPayId: preferences.FcashPayId,
  }, preferences, {
    w: 1,
    upsert: true,
  }, cb);
...
Storage.prototype._completeTxNotesData = function(walletId, notes, cb) {
  ...
  self.fetchWallet(walletId, function(err, wallet) {
    ...
    _.each([].concat(notes), function(note) {
      note.editedByName = wallet.getFcashApp(note.editedBy).name;
    });
...
Storage.prototype.getSession = function(FcashPayId, cb) {
  var self = this;
...
Storage.prototype.fetchPushNotificationSubs = function(FcashPayId, cb) {
...
Storage.prototype.storePushNotificationSub = function(pushNotificationSub, cb) {
  this.db.collection(collections.PUSH_NOTIFICATION_SUBS).update({
    FcashPayId: pushNotificationSub.FcashPayId,
...
Storage.prototype.removePushNotificationSub = function(FcashPayId, token, cb) {
...
Storage.prototype.fetchActiveTxConfirmationSubs = function(FcashPayId, cb) {
  var filter = {
    isActive: true
  };
  if (FcashPayId) filter.FcashPayId = FcashPayId;
...
Storage.prototype.storeTxConfirmationSub = function(txConfirmationSub, cb) {
  this.db.collection(collections.TX_CONFIRMATION_SUBS).update({
    FcashPayId: txConfirmationSub.FcashPayId,
...
Storage.prototype.removeTxConfirmationSub = function(FcashPayId, txid, cb) {
