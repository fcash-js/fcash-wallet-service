...
var collections = {
  ...
  COPAYERS_LOOKUP: 'fcashpay_lookup',
...
Storage.prototype.storeWalletAndUpdateFcashAppLookup = function(wallet, cb) {
  ...
  var fcashpayLookups = _.map(wallet.fcashpay, function(fcashpay) {
    $.checkState(fcashpay.requestPubKeys);
    return {
      FcashPayId: fcashpay.id,
      walletId: wallet.id,
      requestPubKeys: fcashpay.requestPubKeys,
    };
  });
  ...
  this.db.collection(collections.COPAYERS_LOOKUP).remove({
    ...
    if (err) return cb(err);
    self.db.collection(collections.COPAYERS_LOOKUP).insert(fcashpayLookups, {
      w: 1
    }, function(err) {
      if (err) return cb(err);
  ...
  self.fetchWallet(walletId, function(err, wallet) {
    ...
    _.each([].concat(txs), function(tx) {
      ...
      _.each(tx.actions, function(action) {
        action.fcashpayName = wallet.getFcashApp(action.FcashPayId).name;
      });
