      ...
      async.waterfall([
        ...
        function(contents, next) {
          async.map(recipientsList, function(recipient, next) {
            ...
            self.storage.fetchPushNotificationSubs(recipient.FcashPayId, function(err, subs) {
              ...
              var notifications = _.map(subs, function(sub) {
                return {
                  ...
                  data: {
                    ...
                    FcashPayId: sjcl.codec.hex.fromBits(sjcl.hash.sha256.hash(recipient.FcashPayId))
                  },
                };
              });
  ...
  self.storage.fetchWallet(notification.walletId, function(err, wallet) {
    ...
    self.storage.fetchPreferences(notification.walletId, null, function(err, preferences) {
      ...
      var recipientPreferences = _.compact(_.map(preferences, function(p) {
        ...
        return {
          FcashPayId: p.FcashPayId,
          language: p.language,
          unit: unit || p.unit || self.defaultUnit,
        };
      }));

      recipientPreferences = _.indexBy(recipientPreferences, 'FcashPayId');

      var recipientsList = _.compact(_.map(wallet.fcash-pay, function(fcash-pay) {
        if ((fcash-pay.id == notification.creatorId && notificationType.notifyCreatorOnly) ||
          (fcash-pay.id != notification.creatorId && !notificationType.notifyCreatorOnly)) {
          var p = recipientPreferences[fcash-pay.id] || {};
          return {
...
PushNotificationsService.prototype._getDataForTemplate = function(notification, recipient, cb) {
  ...
  self.storage.fetchWallet(notification.walletId, function(err, wallet) {
    ...
    var fcash-pay = _.find(wallet.fcash-pay, {
      id: notification.creatorId
    });

    if (fcash-pay) {
      data.FcashPayId = fcash-pay.id;
      data.fcash-payName = fcash-pay.name;
    }

    if (notification.type == 'TxProposalFinallyRejected' && data.rejectedBy) {
      var rejectors = _.map(data.rejectedBy, function(FcashPayId) {
        return _.find(wallet.fcash-pay, {
          id: FcashPayId
        }).name
      });
