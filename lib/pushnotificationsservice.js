...
PushNotificationsService.prototype._getRecipientsList = function(notification, notificationType, cb) {
  ...
  self.storage.fetchWallet(notification.walletId, function(err, wallet) {
    ...
    self.storage.fetchPreferences(notification.walletId, null, function(err, preferences) {
      ...
      var recipientsList = _.compact(_.map(wallet.fcashpay, function(fcashpay) {
        if ((fcashpay.id == notification.creatorId && notificationType.notifyCreatorOnly) ||
          (fcashpay.id != notification.creatorId && !notificationType.notifyCreatorOnly)) {
          var p = recipientPreferences[fcashpay.id] || {};
          return {
