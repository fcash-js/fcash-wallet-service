...
/**
 * Gets an instance of the server after authenticating the fcashpay.
 * @param {Object} opts
 * @param {string} opts.FcashPayId - The fcashpay id making the request.
 * @param {string} opts.message - (Optional) The contents of the request to be signed. Only needed if no session token is provided.
 * @param {string} opts.signature - (Optional) Signature of message to be verified using one of the fcashpay's requestPubKeys. Only needed if no session token is provided.
 * @param {string} opts.session - (Optional) A valid session token previously obtained using the #login method
 * @param {string} opts.clientVersion - A string that identifies the client issuing the request
 * @param {string} [opts.walletId] - The wallet id to use as current wallet for this request (only when fcashpay is support staff).
 */
WalletService.getInstanceWithAuth = function(opts, cb) {
  function withSignature(cb) {
    ...
    server.storage.fetchFcashAppLookup(opts.FcashPayId, function(err, fcashpay) {
      if (err) return cb(err);
      if (!fcashpay) return cb(new ClientError(Errors.codes.NOT_AUTHORIZED, 'FcashApp not found'));

      if (!fcashpay.isSupportStaff) {
        var isValid = !!server._getSigningKey(opts.message, opts.signature, fcashpay.requestPubKeys);
        if (!isValid)
          return cb(new ClientError(Errors.codes.NOT_AUTHORIZED, 'Invalid signature'));
        server.walletId = fcashpay.walletId;
      } else {
        server.walletId = opts.walletId || fcashpay.walletId;
        server.fcashpayIsSupportStaff = true;
      }
...
WalletService.prototype.login = function(opts, cb) {
  ...
  async.series([
    ...
    if (err) return cb(err);
    if (!session) return cb(new Error('Could not get current session for this fcashpay'));
 ...
 * @param {number} opts.m - Required fcashpay.
 * @param {number} opts.n - Total fcashpay.
 * @param {string} opts.pubKey - Public key to verify fcashpay joining have access to the wallet secret.
...
WalletService.prototype.createWallet = function(opts, cb) {
  ...
  if (!Wallet.verifyFcashAppLimits(opts.m, opts.n))
    return cb(new ClientError('Invalid combination of required fcashpay / total fcashpay'));
 ...
 * @param {Object} opts.includeExtendedInfo - Include PKR info & address managers for wallet & fcashpay
  ...
  async.parallel([
    ...
    function(next) {
      self.getWallet({}, function(err, wallet) {
        ...
        var fcashpayExtendedKeys = ['xPubKey', 'requestPubKey', 'signature', 'addressManager', 'customData'];

        wallet.fcashpay = _.map(wallet.fcashpay, function(fcashpay) {
          if (fcashpay.id == self.FcashPayId) return fcashpay;
          return _.omit(fcashpay, 'customData');
        });
        if (!opts.includeExtendedInfo) {
          wallet = _.omit(wallet, walletExtendedKeys);
          wallet.fcashpay = _.map(wallet.fcashpay, function(fcashpay) {
            return _.omit(fcashpay, fcashpayExtendedKeys);
          });
        }
 ...
 * @param {Object} data
 * @param {Object} opts
 * @param {Boolean} opts.isGlobal - If true, the notification is not issued on behalf of any particular fcashpay (defaults to false)
 */
...
WalletService.prototype._addFcashAppToWallet = function(wallet, opts, cb) {
  ...
  var fcashpay = Model.FcashApp.create({
    coin: wallet.coin,
    name: opts.name,
    fcashpayIndex: wallet.fcashpay.length,
    xPubKey: opts.xPubKey,
    requestPubKey: opts.requestPubKey,
    signature: opts.fcashpaySignature,
    customData: opts.customData,
    derivationStrategy: wallet.derivationStrategy,
  });

  self.storage.fetchFcashAppLookup(fcashpay.id, function(err, res) {
    if (err) return cb(err);
      ...
      async.series([
        ...
        function(next) {
          self._notify('NewFcashApp', {
            ...
            FcashPayId: fcashpay.id,
            fcashpayName: fcashpay.name,
          }, next);
        },
        function(next) {
...
WalletService.prototype._addKeyToFcashApp = function(wallet, fcashpay, opts, cb) {
  var self = this;
  wallet.addFcashAppRequestKey(fcashpay.FcashPayId, opts.requestPubKey, opts.signature, opts.restrictions, opts.name);
  self.storage.storeWalletAndUpdateFcashAppLookup(wallet, function(err) {
    if (err) return cb(err);
...
/**
 * Adds access to a given fcashpay
 *
 * @param {Object} opts
 * @param {string} opts.FcashPayId - The fcashpay id
 * @param {string} opts.requestPubKey - Public Key used to check requests from this fcashpay.
 * @param {string} opts.fcashpaySignature - S(requestPubKey). Used by other fcashpay to verify the that the fcashpay is himself (signed with REQUEST_KEY_AUTH)
...
WalletService.prototype.addAccess = function(opts, cb) {
  ...
  self.storage.fetchFcashAppLookup(opts.FcashPayId, function(err, fcashpay) {
    if (err) return cb(err);
    if (!fcashpay) return cb(Errors.NOT_AUTHORIZED);
    self.storage.fetchWallet(fcashpay.walletId, function(err, wallet) {
      if (err) return cb(err);
      if (!wallet) return cb(Errors.NOT_AUTHORIZED);

      var xPubKey = _.find(wallet.fcashpay, {
        id: opts.FcashPayId
      }).xPubKey;
      ...
      if (fcashpay.requestPubKeys.length > Defaults.MAX_KEYS)
        return cb(Errors.TOO_MANY_KEYS);

      self._addKeyToFcashApp(wallet, fcashpay, opts, cb);
    });
  });
};
...
WalletService.prototype._clientSupportsPayProRefund = function() {
  ...
  if (version.agent != 'fwc') return true;
 ...
 * @param {string} opts.name - The fcashpay name.
 * @param {string} opts.xPubKey - Extended Public Key for this fcashpay.
 * @param {string} opts.requestPubKey - Public Key used to check requests from this fcashpay.
 * @param {string} opts.fcashpaySignature - S(name|xPubKey|requestPubKey). Used by other fcashpay to verify that the fcashpay joining knows the wallet secret.
 * @param {string} opts.customData - (optional) Custom data for this fcashpay.
...
WalletService.prototype.joinWallet = function(opts, cb) {
  ...
  if (!checkRequired(opts, ['walletId', 'name', 'xPubKey', 'requestPubKey', 'fcashpaySignature'], cb)) return;

  if (_.isEmpty(opts.name))
    return cb(new ClientError('Invalid fcashpay name'));
  ...
  self._runLocked(cb, function(cb) {
    self.storage.fetchWallet(opts.walletId, function(err, wallet) {
      ...
      if (!self._verifySignature(hash, opts.fcashpaySignature, wallet.pubKey)) {
        return cb(new ClientError());
      }

      if (_.find(wallet.fcashpay, {
        xPubKey: opts.xPubKey
      })) return cb(Errors.COPAYER_IN_WALLET);

      if (wallet.fcashpay.length == wallet.n) return cb(Errors.WALLET_FULL);
...
/**
 * Save fcashpay preferences for the current wallet/fcashpay pair.
 * @param {Object} opts
...
/**
 * Retrieves a preferences for the current wallet/fcashpay pair.
 * @param {Object} opts
...
/**
 * Verifies that a given message was actually sent by an authorized fcashpay.
 * @param {Object} opts
...
WalletService.prototype.verifyMessageSignature = function(opts, cb) {
  ...
  self.getWallet({}, function(err, wallet) {
    ...
    var fcashpay = wallet.getFcashApp(self.FcashPayId);

    var isValid = !!self._getSigningKey(opts.message, opts.signature, fcashpay.requestPubKeys);
...
/**
 * Publish an already created tx proposal so inputs are locked and other fcashpay in the wallet can see it.
 * @param {Object} opts
 * @param {string} opts.txProposalId - The tx id.
 * @param {string} opts.proposalSignature - S(raw tx). Used by other fcashpay to verify the proposal.
 */
  ...
  self._runLocked(cb, function(cb) {
    self.getWallet({}, function(err, wallet) {
      ...
      self.storage.fetchTx(self.walletId, opts.txProposalId, function(err, txp) {
        ...
        var signingKey = self._getSigningKey(raw, opts.proposalSignature, fcashpay.requestPubKeys);
        if (!signingKey) {
          return cb(new ClientError('Invalid proposal signature'));
        }

        // Save signature info for other fcashpay to check
 ...
 * @param {string} opts.signatures - The signatures of the inputs of this tx for this fcashpay (in apperance order)
 */
WalletService.prototype.signTx = function(opts, cb) {
  ...
  self.getWallet({}, function(err, wallet) {
    ...
    self.getTx({
      ...
      try {
        if (!txp.sign(self.FcashPayId, opts.signatures, fcashpay.xPubKey)) {
 ...
 * @param {Object} opts
 * @param {string} opts.txProposalId - The identifier of the transaction.
 * @param {string} [opts.reason] - A message to other fcashpay explaining the rejection.
 */
    ...
    return _.map(txs, function(tx) {
      ...
      if (proposal) {
        ...
        newTx.actions = _.map(proposal.actions, function(action) {
          return _.pick(action, ['createdOn', 'type', 'FcashPayId', 'fcashpayName', 'comment']);
        });
        ...
        self.storage.storeWallet(wallet, function(err) {
          ...
          _.each([false, true], function(isChange) {
            ...
            if (opts.includeFcashAppBranches) {
              _.each(wallet.fcashpay, function(fcashpay) {
                if (fcashpay.addressManager) {
                  derivators.push({
                    id: fcashpay.addressManager.getBaseAddressPath(isChange),
                    derive: _.bind(fcashpay.createAddress, fcashpay, wallet, isChange),
                    index: _.bind(fcashpay.addressManager.getCurrentIndex, fcashpay.addressManager, isChange),
                    rewind: _.bind(fcashpay.addressManager.rewindIndex, fcashpay.addressManager, isChange),
                  });
                }
              });
            }
...
/**
 * Subscribe this fcashpay to the Push Notifications service using the specified token.
 * @param {Object} opts
...
/**
 * Unsubscribe this fcashpay to the Push Notifications service using the specified token.
 * @param {Object} opts
 * @param {string} opts.token - The token representing the app/device.
 */
...
/**
 * Subscribe this fcashpay to the specified tx to get a notification when the tx confirms.
 * @param {Object} opts
