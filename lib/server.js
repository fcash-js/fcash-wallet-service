...
/**
 * Gets the current version of FWS
 */
WalletService.getServiceVersion = function() {
  if (!serviceVersion)
    serviceVersion = 'fws-' + require('../package').version;
...
WalletService.getInstance = function(opts) {
  ...
  if (version && version.agent == 'bwc') {
    if (version.major == 0 || (version.major == 1 && version.minor < 2)) {
      throw new ClientError(Errors.codes.UPGRADE_NEEDED, 'FWC clients < 1.2 are no longer supported.');
    }
  }
...
/**
 * Gets an instance of the server after authenticating the fcash-pay.
 * @param {Object} opts
 * @param {string} opts.FcashPayId - The fcash-pay id making the request.
 * @param {string} opts.message - (Optional) The contents of the request to be signed. Only needed if no session token is provided.
 * @param {string} opts.signature - (Optional) Signature of message to be verified using one of the fcash-pay's requestPubKeys. Only needed if no session token is provided.
 * @param {string} opts.session - (Optional) A valid session token previously obtained using the #login method
 * @param {string} opts.clientVersion - A string that identifies the client issuing the request
 * @param {string} [opts.walletId] - The wallet id to use as current wallet for this request (only when fcash-pay is support staff).
 */
WalletService.getInstanceWithAuth = function(opts, cb) {
  function withSignature(cb) {
    if (!checkRequired(opts, ['FcashPayId', 'message', 'signature'], cb)) return;
    ...
    server.storage.fetchFcashAppLookup(opts.FcashPayId, function(err, fcash-pay) {
      if (err) return cb(err);
      if (!fcash-pay) return cb(new ClientError(Errors.codes.NOT_AUTHORIZED, 'FcashApp not found'));

      if (!fcash-pay.isSupportStaff) {
        var isValid = !!server._getSigningKey(opts.message, opts.signature, fcash-pay.requestPubKeys);
        if (!isValid)
          return cb(new ClientError(Errors.codes.NOT_AUTHORIZED, 'Invalid signature'));
        server.walletId = fcash-pay.walletId;
      } else {
        server.walletId = opts.walletId || fcash-pay.walletId;
        server.fcash-payIsSupportStaff = true;
      }

      server.FcashPayId = opts.FcashPayId;
      return cb(null, server);
    });
  };

  function withSession(cb) {
    if (!checkRequired(opts, ['FcashPayId', 'session'], cb)) return;
    ...
    server.storage.getSession(opts.FcashPayId, function(err, s) {
      if (err) return cb(err);
...
WalletService.prototype.login = function(opts, cb) {
  ...
  async.series([
    ...
    function(next) {
      self.storage.getSession(self.FcashPayId, function(err, s) {
    ...
    if (err) return cb(err);
    if (!session) return cb(new Error('Could not get current session for this fcash-pay'));
...
WalletService.prototype.logout = function(opts, cb) {
  ...
  self.storage.removeSession(self.FcashPayId, cb);
};
 ...
 * @param {number} opts.m - Required fcash-pay.
 * @param {number} opts.n - Total fcash-pay.
 * @param {string} opts.pubKey - Public key to verify fcash-pay joining have access to the wallet secret.
 * @param {string} opts.singleAddress[=false] - The wallet will only ever have one address.
 * @param {string} opts.coin[='btc'] - The coin for this wallet (btc, bch).
 * @param {string} opts.network[='livenet'] - The Fcash network for this wallet.
...
WalletService.prototype.createWallet = function(opts, cb) {
  ...
  if (!Wallet.verifyFcashAppLimits(opts.m, opts.n))
    return cb(new ClientError('Invalid combination of required fcash-pay / total fcash-pay'));
 ...
 * @param {Object} opts.includeExtendedInfo - Include PKR info & address managers for wallet & fcash-pay
  ...
  async.parallel([
    ...
    function(next) {
      self.getWallet({}, function(err, wallet) {
        ...
        var fcash-payExtendedKeys = ['xPubKey', 'requestPubKey', 'signature', 'addressManager', 'customData'];

        wallet.fcash-pay = _.map(wallet.fcash-pay, function(fcash-pay) {
          if (fcash-pay.id == self.FcashPayId) return fcash-pay;
          return _.omit(fcash-pay, 'customData');
        });
        if (!opts.includeExtendedInfo) {
          wallet = _.omit(wallet, walletExtendedKeys);
          wallet.fcash-pay = _.map(wallet.fcash-pay, function(fcash-pay) {
            return _.omit(fcash-pay, fcash-payExtendedKeys);
          });
        }
 ...
 * @param {Object} data
 * @param {Object} opts
 * @param {Boolean} opts.isGlobal - If true, the notification is not issued on behalf of any particular fcash-pay (defaults to false)
 */
WalletService.prototype._notify = function(type, data, opts, cb) {
  ...
  var FcashPayId = self.FcashPayId || data.FcashPayId;
  ...
  var notification = Model.Notification.create({
    ...
    creatorId: opts.isGlobal ? null : FcashPayId,
    walletId: walletId,
  });
...
WalletService.prototype._addFcashAppToWallet = function(wallet, opts, cb) {
  var self = this;

  var fcash-pay = Model.FcashApp.create({
    coin: wallet.coin,
    name: opts.name,
    fcash-payIndex: wallet.fcash-pay.length,
    xPubKey: opts.xPubKey,
    requestPubKey: opts.requestPubKey,
    signature: opts.fcash-paySignature,
    customData: opts.customData,
    derivationStrategy: wallet.derivationStrategy,
  });

  self.storage.fetchFcashAppLookup(fcash-pay.id, function(err, res) {
    if (err) return cb(err);
    ...
    self.storage.storeWalletAndUpdateFcashAppLookup(wallet, function(err) {
      if (err) return cb(err);

      async.series([

        function(next) {
          self._notify('NewFcashApp', {
            walletId: opts.walletId,
            FcashPayId: fcash-pay.id,
            fcash-payName: fcash-pay.name,
          }, next);
        },
        function(next) {
...
WalletService.prototype._addKeyToFcashApp = function(wallet, fcash-pay, opts, cb) {
  var self = this;
  wallet.addFcashAppRequestKey(fcash-pay.FcashPayId, opts.requestPubKey, opts.signature, opts.restrictions, opts.name);
  self.storage.storeWalletAndUpdateFcashAppLookup(wallet, function(err) {
    if (err) return cb(err);
...
/**
 * Adds access to a given fcash-pay
 *
 * @param {Object} opts
 * @param {string} opts.FcashPayId - The fcash-pay id
 * @param {string} opts.requestPubKey - Public Key used to check requests from this fcash-pay.
 * @param {string} opts.fcash-paySignature - S(requestPubKey). Used by other fcash-pay to verify the that the fcash-pay is himself (signed with REQUEST_KEY_AUTH)
...
WalletService.prototype.addAccess = function(opts, cb) {
  ...
  if (!checkRequired(opts, ['FcashPayId', 'requestPubKey', 'signature'], cb)) return;

  self.storage.fetchFcashAppLookup(opts.FcashPayId, function(err, fcash-pay) {
    if (err) return cb(err);
    if (!fcash-pay) return cb(Errors.NOT_AUTHORIZED);
    self.storage.fetchWallet(fcash-pay.walletId, function(err, wallet) {
      if (err) return cb(err);
      if (!wallet) return cb(Errors.NOT_AUTHORIZED);

      var xPubKey = _.find(wallet.fcash-pay, {
        id: opts.FcashPayId
      }).xPubKey;
      ...
      if (fcash-pay.requestPubKeys.length > Defaults.MAX_KEYS)
        return cb(Errors.TOO_MANY_KEYS);

      self._addKeyToFcashApp(wallet, fcash-pay, opts, cb);
    });
  });
};
...
WalletService._getFcashAppHash = function(name, xPubKey, requestPubKey) {
 ...
 * @param {string} opts.name - The fcash-pay name.
 * @param {string} opts.xPubKey - Extended Public Key for this fcash-pay.
 * @param {string} opts.requestPubKey - Public Key used to check requests from this fcash-pay.
 * @param {string} opts.fcash-paySignature - S(name|xPubKey|requestPubKey). Used by other fcash-pay to verify that the fcash-pay joining knows the wallet secret.
 * @param {string} opts.customData - (optional) Custom data for this fcash-pay.
...
WalletService.prototype.joinWallet = function(opts, cb) {
  ...
  if (!checkRequired(opts, ['walletId', 'name', 'xPubKey', 'requestPubKey', 'fcash-paySignature'], cb)) return;

  if (_.isEmpty(opts.name))
    return cb(new ClientError('Invalid fcash-pay name'));
  ...
  self._runLocked(cb, function(cb) {
    self.storage.fetchWallet(opts.walletId, function(err, wallet) {
      ...
      if (!self._verifySignature(hash, opts.fcash-paySignature, wallet.pubKey)) {
        return cb(new ClientError());
      }

      if (_.find(wallet.fcash-pay, {
        xPubKey: opts.xPubKey
      })) return cb(Errors.COPAYER_IN_WALLET);

      if (wallet.fcash-pay.length == wallet.n) return cb(Errors.WALLET_FULL);

      self._addFcashAppToWallet(wallet, opts, cb);
    });
  });
};

/**
 * Save fcash-pay preferences for the current wallet/fcash-pay pair.
 * @param {Object} opts
 * @param {string} opts.email - Email address for notifications.
 * @param {string} opts.language - Language used for notifications.
 * @param {string} opts.unit - Fcash unit used to format amounts in notifications.
 */
WalletService.prototype.savePreferences = function(opts, cb) {
  ...
  self._runLocked(cb, function(cb) {
    self.storage.fetchPreferences(self.walletId, self.FcashPayId, function(err, oldPref) {
      if (err) return cb(err);
...
/**
 * Retrieves a preferences for the current wallet/fcash-pay pair.
 * @param {Object} opts
...
WalletService.prototype.getPreferences = function(opts, cb) {
  ...
  self.storage.fetchPreferences(self.walletId, self.FcashPayId, function(err, preferences) {
    if (err) return cb(err);
...
/**
 * Verifies that a given message was actually sent by an authorized fcash-pay.
 * @param {Object} opts
...
WalletService.prototype.verifyMessageSignature = function(opts, cb) {
  ...
  self.getWallet({}, function(err, wallet) {
    ...
    var fcash-pay = wallet.getFcashApp(self.FcashPayId);

    var isValid = !!self._getSigningKey(opts.message, opts.signature, fcash-pay.requestPubKeys);
 ...
 * @param {string} [opts.network = 'livenet'] - The Fcash network to estimate fee levels from.
...
WalletService.prototype._checkTx = function(txp) {
  ...
  try {
    var fcashBaseTx = txp.getFcashTx();
    fcashBaseError = fcashBaseTx.getSerializationError(serializationOpts);
    if (!fcashBaseError) {
      txp.fee = fcashBaseTx.getFee();
    }
  } catch (ex) {
  ...
  if (fcashBaseError instanceof Fcash_[txp.coin].errors.Transaction.FeeError)
    return Errors.INSUFFICIENT_FUNDS_FOR_FEE;

  if (fcashBaseError instanceof Fcash_[txp.coin].errors.Transaction.DustOutputs)
    return Errors.DUST_AMOUNT;
  return fcashBaseError;
};
...
WalletService.prototype._canCreateTx = function(cb) {
  var self = this;
  self.storage.fetchLastTxs(self.walletId, self.FcashPayId, 5 + Defaults.BACKOFF_OFFSET, function(err, txs) {
    if (err) return cb(err);
...
/**
 * Publish an already created tx proposal so inputs are locked and other fcash-pay in the wallet can see it.
 * @param {Object} opts
 * @param {string} opts.txProposalId - The tx id.
 * @param {string} opts.proposalSignature - S(raw tx). Used by other fcash-pay to verify the proposal.
 */
  ...
  self._runLocked(cb, function(cb) {
    self.getWallet({}, function(err, wallet) {
      ...
      self.storage.fetchTx(self.walletId, opts.txProposalId, function(err, txp) {
        ...
        var signingKey = self._getSigningKey(raw, opts.proposalSignature, fcash-pay.requestPubKeys);
        if (!signingKey) {
          return cb(new ClientError('Invalid proposal signature'));
        }

        // Save signature info for other fcash-pay to check
  ...
  self._runLocked(cb, function(cb) {
    self.storage.fetchTxNote(self.walletId, opts.txid, function(err, note) {
      ...
      if (!note) {
        ...
        note.edit(opts.body, self.FcashPayId);
      }
...
WalletService.prototype.getRemainingDeleteLockTime = function(txp) {
  ...
  if (txp.creatorId !== this.FcashPayId)
    return lockTimeRemaining;

  // has other approvers? need to wait
  var approvers = txp.getApprovers();
  if (approvers.length > 1 || (approvers.length == 1 && approvers[0] !== this.FcashPayId))
    return lockTimeRemaining;
 ...
 * @param {string} [opts.network = 'livenet'] - The Fcash network for this transaction.
 ...
 * @param {string} opts.signatures - The signatures of the inputs of this tx for this fcash-pay (in apperance order)
 */
WalletService.prototype.signTx = function(opts, cb) {
  ...
  self.getWallet({}, function(err, wallet) {
    ...
    self.getTx({
      ...
      try {
        if (!txp.sign(self.FcashPayId, opts.signatures, fcash-pay.xPubKey)) {
 ...
 * @param {Object} opts
 * @param {string} opts.txProposalId - The identifier of the transaction.
 * @param {string} [opts.reason] - A message to other fcash-pay explaining the rejection.
 */
WalletService.prototype.rejectTx = function(opts, cb) {
  ...
  self.getTx({
    ...
    txp.reject(self.FcashPayId, opts.reason);
    ...
    return _.map(txs, function(tx) {
      ...
      if (proposal) {
        ...
        newTx.actions = _.map(proposal.actions, function(action) {
          return _.pick(action, ['createdOn', 'type', 'FcashPayId', 'fcash-payName', 'comment']);
        });
 ...
 *
 * @param {Object} opts
 * @param {Boolean} opts.includeFcashAppBranches (defaults to false)
 */
      ...
      self.storage.clearTxHistoryCache(self.walletId, function() {
        self.storage.storeWallet(wallet, function(err) {
          ...
          _.each([false, true], function(isChange) {
            ...
            if (opts.includeFcashAppBranches) {
              _.each(wallet.fcash-pay, function(fcash-pay) {
                if (fcash-pay.addressManager) {
                  derivators.push({
                    id: fcash-pay.addressManager.getBaseAddressPath(isChange),
                    derive: _.bind(fcash-pay.createAddress, fcash-pay, wallet, isChange),
                    index: _.bind(fcash-pay.addressManager.getCurrentIndex, fcash-pay.addressManager, isChange),
                    rewind: _.bind(fcash-pay.addressManager.rewindIndex, fcash-pay.addressManager, isChange),
                  });
                }
              });
            }
...
/**
 * Subscribe this fcash-pay to the Push Notifications service using the specified token.
 * @param {Object} opts
...
/**
 * Unsubscribe this fcash-pay to the Push Notifications service using the specified token.
 * @param {Object} opts
 * @param {string} opts.token - The token representing the app/device.
 */
WalletService.prototype.pushNotificationsUnsubscribe = function(opts, cb) {
  ...
  self.storage.removePushNotificationSub(self.FcashPayId, opts.token, cb);
};

/**
 * Subscribe this fcash-pay to the specified tx to get a notification when the tx confirms.
 * @param {Object} opts
...
WalletService.prototype.txConfirmationUnsubscribe = function(opts, cb) {
  ...
  self.storage.removeTxConfirmationSub(self.FcashPayId, opts.txid, cb);
};
