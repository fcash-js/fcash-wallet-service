...
WalletService.prototype._checkTx = function(txp) {
  ...
  try {
    var fcoreTx = txp.getFcashTx();
    fcoreError = fcoreTx.getSerializationError(serializationOpts);
    if (!fcoreError) {
      txp.fee = fcoreTx.getFee();
    }
  } catch (ex) {
  ...
  if (fcoreError instanceof Fcash_[txp.coin].errors.Transaction.FeeError)
    return Errors.INSUFFICIENT_FUNDS_FOR_FEE;

  if (fcoreError instanceof Fcash_[txp.coin].errors.Transaction.DustOutputs)
