...
var crypto = fcash_base.crypto;
var encoding = fcash_base.encoding;
...
Utils.hashMessage = function(text, noReverse) {
  ...
  if (!noReverse) {
    ret = new fcash_base.encoding.BufferReader(ret).readReverse();
  }
  return ret;
};
...
Utils.parseVersion = function(version) {
  ...
  v.agent = _.contains(['bwc', 'fws'], x[0]) ? 'bwc' : x[0];
  x = x[1].split('.');
