...
ExpressApp.prototype.start = function(opts, cb) {
  ...
  function getServerWithAuth(req, res, opts, cb) {
    ...
    var auth = {
      FcashPayId: credentials.FcashPayId,
    ...
    WalletService.getInstanceWithAuth(auth, function(err, server) {
      ...
      if (opts.onlySupportStaff && !server.fcash-payIsSupportStaff) {
        return returnError(new WalletService.ClientError({
          code: 'NOT_AUTHORIZED'
        }), res, req);
      }
      ...
      req.FcashPayId = server.FcashPayId;
      return cb(server);
    });
  };
  ...
  router.put('/v1/fcash-pay/:id/', function(req, res) {
    req.body.FcashPayId = req.params['id'];
    var server;
    try {
      server = getServer(req, res);
    } catch (ex) {
  ...
  // DEPRECATED
  router.post('/v1/wallets/:id/fcash-pay/', function(req, res) {
    logDeprecated(req);
  ...
  router.post('/v2/wallets/:id/fcash-pay/', function(req, res) {
    req.body.walletId = req.params['id'];
    var server;
    try {
      server = getServer(req, res);
  ...
  this.app.use(opts.basePath || '/fws/api', router);
