...
ExpressApp.prototype.start = function(opts, cb) {
  ...
  function getServerWithAuth(req, res, opts, cb) {
    ...
    WalletService.getInstanceWithAuth(auth, function(err, server) {
      ...
      if (opts.onlySupportStaff && !server.fcashpayIsSupportStaff) {
        return returnError(new WalletService.ClientError({
          code: 'NOT_AUTHORIZED'
        }), res, req);
      }
  ...
  router.put('/v1/fcashpay/:id/', function(req, res) {
  ...
  // DEPRECATED
  router.post('/v1/wallets/:id/fcashpay/', function(req, res) {
    logDeprecated(req);
  ...
  router.post('/v2/wallets/:id/fcashpay/', function(req, res) {
    req.body.walletId = req.params['id'];
    var server;
    try {
      server = getServer(req, res);
